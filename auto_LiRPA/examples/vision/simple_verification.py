...
lirpa_model.visualize("bounded_mnist_model.png")
print()
...
original_images_cpu = image.cpu().clone()
for i in range(N):
    save_image(original_images_cpu[i], f'verification_images/original_image_{i}_label_{true_label[i]}.png')
print(f'Saved {N} original images to verification_images/ directory\n')

# Generate and save perturbed image samples for analysis
num_samples = 5
for i in range(N):
    img_samples = []
    img_tensor = image[i:i+1]  # Keep on same device as image
    for sample_idx in range(num_samples):
        if norm == float("inf"):
            # L∞ perturbation: uniform sampling
            noise = torch.rand_like(img_tensor) * 2 * eps - eps  # [-eps, eps]
            perturbed = torch.clamp(img_tensor + noise, 0, 1)
        else:
            # L2 or other norms: normalized random noise
            noise = torch.randn_like(img_tensor)
            noise = noise / torch.norm(noise.view(-1), p=norm) * eps
            perturbed = torch.clamp(img_tensor + noise, 0, 1)
        img_samples.append(perturbed.cpu())
    
    # Save all samples in a grid
    save_image(torch.cat(img_samples, dim=0), 
               f'verification_images/perturbed_samples_image_{i}_label_{true_label[i]}_eps_{eps}.png',
               nrow=num_samples)
print(f'Saved {num_samples} perturbed samples per image to verification_images/ directory\n')
